...
if __name__ == "__main__":
    ...
    for i_row, DB in enumerate(DB_LIST):
        ...
        plot_x_label = DB == "deep10m"
        plot_ax_title = DB == "sift"
        for i_col, t_rng in enumerate(RANGE_FRACTIONS):
            ax = axes.flatten()[i_row * num_cols + i_col]
            final_y_ticks, final_y_ticks_labels = [], []
            for i, method in enumerate(METHOD_LIST):
                csv_file = os.path.join(DB_DIR, method, f"{t_rng}.csv")
                if not os.path.exists(csv_file):
                ...
                # reshape
                if (
                    method in OURS_LAYERED
                    or method in OURS_PP
                    or method == "oracle_hnsw"
                    or method in HSIG
                    or method == "postfiltering"
                ):
                    data = data.reshape((-1, 7))

                elif method in IRANGE or method in SERF or method in WST or method in DIGRA:
                    data = data.reshape((-1, 5))
                elif method in ACORN or method in MILVUS or method in RANGE_PQ:
                    data = data.reshape((-1, 3))
                else:
            ...
            if plot_x_label:
                ax.set_xlabel(x_label, fontsize=18, labelpad=0.5)
            if i_col == 0:
            ...
            if plot_ax_title:
                if t_rng == 17:
                    ax.set_title(r"Mixed range fraction", fontsize=20, pad=0.5)
                else:
                    ax.set_title(r"Range fraction: $2^{" + f"-{t_rng}" + "}$", fontsize=20)
    ...
    # push prefilter and postfilter to the end of the legend
    # reorder the handles and labels lists
    
    if get_method_beautify("prefiltering") in labels:
        idx = labels.index(get_method_beautify("prefiltering"))
        handles.append(handles.pop(idx))
        labels.append(labels.pop(idx))
    if get_method_beautify("postfiltering") in labels:
        idx = labels.index(get_method_beautify("postfiltering"))
        handles.append(handles.pop(idx))
        labels.append(labels.pop(idx))

    # push RangePQ before DIGRA in labels and handles
    if "RangePQ" in labels and "DIGRA" in labels:
        idx_rpq = labels.index("RangePQ")
        idx_digra = labels.index("DIGRA")
        if idx_rpq > idx_digra:
            handles.insert(idx_digra, handles.pop(idx_rpq))
            labels.insert(idx_digra, labels.pop(idx_rpq))
    print("Final legend labels:", labels)
    ...
    plt.tight_layout()
    fig.subplots_adjust(hspace=0.16, wspace=0.17)
    res_file = f"qps-recall-combined_plot.pdf"
