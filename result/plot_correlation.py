...
if __name__ == "__main__":
    ...
    def method_filter(method):
        ...
        if method in OURS_PP or method == "prefiltering" or method == "postfiltering":
            return True
        return False
    ...
    for i_row, DB in enumerate(DB_LIST):
        plot_x_label = DB == "sift"
        plot_ax_title = DB == "sift"
        recall_list = [0.1, 0.4, 0.8]
        for i_col, level in enumerate(["low", "mid", "high"]):
            RECALL_THRESHOLD = recall_list[i_col]
            DB_DIR = f"{DB}_correlation_k{K}"
            ax = axes.flatten()[i_row * num_cols + i_col]
            final_y_ticks, final_y_ticks_labels = [], []
            for i, method in enumerate(METHOD_LIST):
                if not method_filter(method):
                    continue
                csv_file = os.path.join(DB_DIR, method, f"{level}.csv")
                if not os.path.exists(csv_file):
                ...
                elif method in IRANGE or method in SERF or method in WST or method in DIGRA or method in DSG:
                    data = data.reshape((-1, 5))
            ...
            if plot_x_label:
                ax.set_xlabel(x_label, fontsize=25, labelpad=0.2)
            if i_col == 0:
            ...
            if plot_ax_title:
                title_map = {"low": "Negative correlation", "mid": "No correlation", "high": "Positive correlation"}
                ax.set_title(
                    f"{title_map[level]}",
                    fontsize=25,
                    pad=0.5,
    ...
    res_file = f"qps-recall-correlation.pdf"
