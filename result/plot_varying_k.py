...
if __name__ == "__main__":
    ...
    def method_filter(method):
        ...
        if method in OURS_PP or method in IRANGE or method in HSIG or method in DIGRA:
            return True
        return False
    ...
    for i_row, DB in enumerate(DB_LIST):
        ...
        for i_col, k in enumerate([1,50,100]):
            ...
            for i, method in enumerate(METHOD_LIST):
                if not method_filter(method):
                    continue
                csv_file = os.path.join(DB_DIR, method, f"{17}.csv")
                if not os.path.exists(csv_file):
            ...
            ticks_size = 14.5 if i_row == 1 else 14
            ax.tick_params(axis="y", which="major", labelsize=ticks_size, pad=0.2)
            ax.tick_params(axis="x", which="major", labelsize=18, pad=0.2)
            if plot_x_label:
                ax.set_xlabel(x_label, fontsize=25, labelpad=0.5)
            if i_col == 0:
            ...
            # if plot_ax_title:
            #     ax.set_title(r"$k$=" + f"{k}", fontsize=20, pad=0.5)
    ...
    res_file = f"qps-recall-varying-k.pdf"
