...
void RunSearching(const argparse::ArgumentParser &program, spatt::SearchParameters sp, spatt::DataLoader<vec_t> &dl,
    spatt::GTManager<vec_t> &gt_manager, spatt::SpattIndexSearcher<vec_t> &searcher)
{
...
int main(int args, char *argv[])
{
  ...
  spatt::SpattIndexSearcher<float> searcher(&dl, program.get<std::string>("space"));
  searcher.LoadIndex(program.get<std::string>("index"));
#ifdef DEBUG
  searcher.SetGTManager(&gt_manager);
#endif
