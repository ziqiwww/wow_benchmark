...
class SpattIndexSearcher : public SpattIndex<vec_t>
{
public:
  SpattIndexSearcher(DataLoader<vec_t> *dl, const std::string &space);
...
SpattIndexSearcher<vec_t>::SpattIndexSearcher(DataLoader<vec_t> *dl, const std::string &space)
    : SpattIndex<vec_t>(dl, space)
{}

template <typename vec_t>
auto SpattIndexSearcher<vec_t>::Search(
    const vec_t *query_vec, const QueryFilter<label_t> &filter, SearchParameters &sparam) -> SearchStatus
{
#ifdef DEBUG
  LOG(fmt::format("Searching with ef: {}, filter range: [{}, {}]", sparam.efs_, filter.l_, filter.u_));
#endif
  SearchStatus status;
  ...
  auto result = this->template SearchCandidates<false>(
      eps, query_vec, filter, sparam.layer_rng_, sparam.efs_, status.rt_status_, true);
...
template <typename vec_t>
auto SpattIndexSearcher<vec_t>::DecideLayerRange(const QueryFilter<label_t> &filter_rng) -> QueryFilter<int>
{
