...
int main(int argc, char *argv[])
{
  ...
  program.add_argument("-ba", "--base-attr").help("Base attribute path").default_value("").required();
  ...
  program.add_argument("-mul", "--k-multiplier")
      .help("Multiplier for k in postfiltering")
      .default_value(3)
      .scan<'u', size_t>();
  ...
  for (int i = 0; i < num_query; ++i) {
    ...
    auto                             start = std::chrono::high_resolution_clock::now();
    for (int j = 0; j < dl.nb_; ++j) {
      if (dl.base_meta_[j] < rng.l_ || dl.base_meta_[j] > rng.u_) {
        continue;  // skip if the base vector is not in the range
      }
  ...
  for (auto efs : efs) {
    ...
    for (size_t i = 0; i < num_query; ++i) {
      ...
      hnsw->metric_hops                  = 0;
      size_t k_multiplier                = c / (rng.u_ - rng.l_ + 1) + 1;
      efs = std::max(efs, k_multiplier * program.get<size_t>("k"));
      hnsw->setEf(efs);
      auto                             start      = std::chrono::high_resolution_clock::now();
      auto                             result_que = hnsw->searchKnn(query, k_multiplier * program.get<size_t>("k"));
      auto                             end        = std::chrono::high_resolution_clock::now();
      std::vector<spatt::dist_id_pair> result;
      result.reserve(result_que.size());
      while (!result_que.empty()) {
        auto [dist, id] = result_que.top();
        if (dl.base_meta_[id] < rng.l_ || dl.base_meta_[id] > rng.u_) {
          result_que.pop();
          continue;  // skip if the base vector is not in the range
        }
